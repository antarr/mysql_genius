RSpec.describe MysqlGenius::SqlValidator do
  let(:blocked_tables) { %w[sessions authentication_tokens] }
  let(:all_tables) { %w[users posts sessions authentication_tokens] }
  let(:connection) do
  ...
  describe ".masked_column?" do
    let(:patterns) { %w[password secret digest token] }
