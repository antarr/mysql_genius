...
RSpec.describe("Analysis routes", type: :request) do
  ...
  it "GET /mysql_genius/primary/duplicate_indexes returns 200 + JSON array" do
    get "/mysql_genius/primary/duplicate_indexes"
    expect(last_response).to(be_ok)
    expect(JSON.parse(last_response.body)).to(be_an(Array))
  end

  it "GET /mysql_genius/primary/table_sizes returns 200 + JSON array" do
    get "/mysql_genius/primary/table_sizes"
    expect(last_response).to(be_ok)
    expect(JSON.parse(last_response.body)).to(be_an(Array))
  end

  it "GET /mysql_genius/primary/query_stats returns 200 + JSON array" do
    get "/mysql_genius/primary/query_stats"
    expect(last_response).to(be_ok)
    expect(JSON.parse(last_response.body)).to(be_an(Array))
  end

  it "GET /mysql_genius/primary/unused_indexes returns 200 + JSON array" do
    get "/mysql_genius/primary/unused_indexes"
    expect(last_response).to(be_ok)
    expect(JSON.parse(last_response.body)).to(be_an(Array))
  end

  it "GET /mysql_genius/primary/server_overview returns 200 + JSON object" do
    get "/mysql_genius/primary/server_overview"
    expect(last_response).to(be_ok)
  ...
  it "does not raise NoMethodError on any analysis route (boot-order regression guard)" do
    ...
    ["duplicate_indexes", "table_sizes", "query_stats", "unused_indexes", "server_overview", "slow_queries"].each do |route|
      expect { get("/mysql_genius/primary/#{route}") }.not_to(raise_error)
